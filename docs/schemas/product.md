...
- `type` stays an open `str` with its `"external"` default, following the JSON Schema, even though upstream's prose calls `external` "the default and only supported value". The declared source of truth for this port is `odps-json-schema-v1.0.0.json`, which supplies a `default` but no `enum`; closing the union would also reject `test/odps.full.example.yaml`, which deliberately exercises a second value. The prose caveat is carried in the attribute's docstring instead.
...
- If upstream promotes its `external`-only prose into an actual `enum`, close `Sbom.type` then — and update the full example fixture in the same change.
